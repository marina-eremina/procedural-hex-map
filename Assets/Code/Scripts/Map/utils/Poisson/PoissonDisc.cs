...
public class PoissonDisk { 
        ...
        var bags = new Bags()
            {
                ...
                Grid = new Vector2?[(int)settings.GridWidth + 1, (int)settings.GridHeight + 1],
        ...
        private static bool GetNextPoint(Vector2 point, Settings set, Bags bags)
        {
            ...
            var around = 2;
            var fieldMin = new Vector2(Mathf.Max(0, index.x - around), Mathf.Max(0, index.y - around));
            var fieldMax = new Vector2(Mathf.Min(set.GridWidth, index.x + around), Mathf.Min(set.GridHeight, index.y + around));

            for (var i = fieldMin.x; i <= fieldMax.x && drop == false; i++)
            {
                for (var j = fieldMin.y; j <= fieldMax.y && drop == false; j++)
                {
                    var q = bags.Grid[(int)i, (int)j];
            ...
            if (drop == false)
            {
                ...
                bags.Grid[(int)index.x, (int)index.y] = p;
            }
        ...
        private static void GetFirstPoint(Settings set, Bags bags)
        {
            ...
            bags.Grid[(int)index.x, (int)index.y] = first;
        ...
        #region "Utils"
        private static Vector2 GetGridIndex(Vector2 point, Settings set)
        {
            return new Vector2(
        ...
        public static Vector2f ConvertToVector2f(Vector2 vector2)
        {
            return new Vector2f(vector2.x, vector2.y);
        }

        public static List<Vector2f> ConvertToListVector2f(List<Vector2> vector2List)
        {
            List<Vector2f> vector2fList = new List<Vector2f>(vector2List.Count);

            foreach (Vector2 vec2 in vector2List)
            {
                vector2fList.Add(new Vector2f(vec2.x, vec2.y));
            }
